...
def health():
    try:
        ...
        return jsonify({"ok": True, "time": datetime.now(timezone.utc).isoformat() + "Z", "db": "connected"})
...
def login():
    ...
    # find user by _id (email)
    user = Vartotojai.find_one({"_id": email})
...
def register_user():
    ...
    if Vartotojai.find_one({"_id": email}):
        return app.response_class(
    ...
    Vartotojai.insert_one(user_doc)
...
def purchase():
    ...
    if not Vartotojai.find_one({"_id": vartotojo_id}):
        return app.response_class(
    ...
    with client.start_session() as s:
        with s.start_transaction():
            ev = Renginiai.find_one({"_id": renginys_id}, session=s)
            if not ev:
                return app.response_class(
